...
#include "GameScene\Module\FlowCtrl.h"
...
bool SaveSelect::init()
{
	...
	auto blur = LayerColor::create(Color4B(49, 49, 49, 180));
	this->addChild(blur);
	...
	if (fileUtils->isFileExist("Save/0.sav")){
		...
		label1->setString(str["day"].asString() + " day(s)");
	}
	...
	if (fileUtils->isFileExist("Save/1.sav")){
		...
		label2->setString(str["day"].asString() + " day(s)");
	}
	...
	if (fileUtils->isFileExist("Save/2.sav")){
		...
		label3->setString(str["day"].asString() + " day(s)");
	}

	_return = ui::Button::create("UI/Button/Return.png", "UI/Button/Return_Click.png");
...
{
	FlowCtrl::getInstance()->newGame(num, _saveType);
	FlowCtrl::getInstance()->startGame();
}
